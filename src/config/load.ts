...
export async function loadPolicy(
  ...
  const policyFilePath = environment["OS_BATCH_POLICY_FILE"];
  const loaded =
  ...
  const workspaceOverride = environment["OS_BATCH_WORKSPACE_ROOT"];
  ...
  const logLevelOverride = environment["OS_BATCH_LOG_LEVEL"];
  ...
  const readOnlyOverride = parseBoolean(
    "OS_BATCH_READ_ONLY",
    environment["OS_BATCH_READ_ONLY"],
  );
